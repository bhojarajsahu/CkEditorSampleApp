...
watchdog.setCreator( ( element, configuration ) => {
	return CKSource.Editor
		...
		.then( editor => {
			...
			function enableShareButton() {
				...
				editor.ui.view.listenTo( inputElement, 'click', () => inputElement.select() );
				editor.ui.view.listenTo( triggerElement, 'click', () => copyFromInput( inputElement, triggerElement ) );
			}
...
watchdog.setDestructor( editor => {
	...
	loaderElement.classList.remove( 'fadeout' );

	if ( !loaderElement.parentNode ) {
		loaderElementContainer.appendChild( loaderElement );
	}

	document.querySelector( '.collaboration-demo' ).classList.remove( 'collaboration-demo--ready' );
...
createDialog().then( config => watchdog.create(
	document.querySelector( '.collaboration-demo__editable' ), {
...
function handleSampleError( error ) {
	...
	const message = [
		...
		`Please, report the following error on ${ issueUrl } with the build id "hz1tonnujpk4-cdv419tyyiw8" and the error stack trace:`
	].join( '\n' );
